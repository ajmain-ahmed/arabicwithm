-- Restore Memory XP awards (reverts the behaviour change in 20260910180000_memory_no_xp.sql).
-- Awards p_xp once per distinct card per day; non-premium users are capped at
-- p_daily_limit reviewed cards per day; premium bypasses the cap.
-- Only service_role may execute this function; the website sends MEMORY.xpPerCard.
begin;
