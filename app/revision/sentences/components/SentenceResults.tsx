...
import type { ExtendedSessionLog, Queue } from '../types'
import { QUEUE_CONFIG, classifyCard } from '../types'
import type { SentenceRevisionCard } from '@/app/actions/revision'

interface SentenceResultsProps {
    logs: ExtendedSessionLog[]
    priorCompleted: SentenceRevisionCard[]
    onRestart: () => void
    isLoading?: boolean
    sessionMode?: 'daily' | 'custom' | null
}

export default function SentenceResults({
    logs,
    priorCompleted,
    onRestart,
    isLoading = false,
    sessionMode,
}: SentenceResultsProps) {
    ...
    if (isDaily) priorCompleted.forEach(c => shows.add(c.show_slug))
    logs.forEach(l => {
        const [show] = l.cardId.split(':')
            ...
            <Container maxWidth="sm" sx={{ py: { xs: 4, md: 6 } }}>
                <Box sx={{
                    ...
                    <Typography sx={{
                        ...
                        {isDaily ? 'sentences completed today' : 'sentences practiced'}
                    </Typography>
