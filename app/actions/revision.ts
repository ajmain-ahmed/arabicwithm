...
/**
 * Returns the start of today in UTC (00:00:00.000Z).
 */
...
export async function fetchDueRevisionCards(): Promise<RevisionCard[]> {
  ...
  // 2. Brand new cards (never reviewed) – limited per day, respecting already introduced cards
  ...
  // 3. Learning cards (seen today but not yet graduated) – only those currently due
  const { data: learning, error: learningError } = await serviceClient
    ...
    .lte('next_review_at', now)   // ✅ only due learning cards

  if (learningError) throw new Error(learningError.message)
  console.log("[revision] learning count:", learning?.length ?? 0)

  // Combine: reviews first, then brand new, then other learning cards
...
export async function submitRevisionAnswer(
  ...
  const updates: any = {
    repetitions: graduated ? newReps : 0,
    interval_days: graduated ? newInterval : 0,
    ease_factor: newEase,
    last_review_at: now,
    next_review_at: nextReview.toISOString(),
  }

  if (graduated && updates.repetitions === 0) {
    updates.repetitions = 1
  }
